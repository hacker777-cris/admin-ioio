...
const Dashboard: React.FC = () => {
  ...
  const generatePropertyData = () => {
    ...
    return [
      { name: "Active", value: propertyStats.active || 0 },
      { name: "Pending", value: propertyStats.pending || 0 },
      { name: "Featured", value: propertyStats.featured || 0 },
    ];
  };

  // Helper function to safely calculate total pending moderation items
  const getTotalPendingModeration = () => {
    if (!moderationStats) return 0;

    return (
      (moderationStats.message_reports?.pending || 0) +
      (moderationStats.flagged_messages?.pending || 0) +
      (moderationStats.owner_reports?.pending || 0) +
      (moderationStats.reviews?.flagged || 0)
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" icon={<Clock size={16} />}>
            Last 30 days
          </Button>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            trend={{
              value:
                userStats.total > 0
                  ? Math.round(
                      ((userStats.new_this_month || 0) / userStats.total) * 100,
                    )
                  : 0,
        ...
        {moderationStats && (
          <StatCard
            ...
            value={getTotalPendingModeration()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    (moderationStats.message_reports?.pending || 0) > 0
                      ? "bg-yellow-100 text-yellow-800"
                      : "bg-green-100 text-green-800"
                  }`}
                >
                  {moderationStats.message_reports?.pending || 0} Pending
                </span>
              </div>
              <p className="text-2xl font-semibold">
                {moderationStats.message_reports?.total || 0}
              </p>
              <p className="text-sm text-gray-500">
                {moderationStats.message_reports?.resolved || 0} resolved
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    (moderationStats.flagged_messages?.pending || 0) > 0
                      ? "bg-yellow-100 text-yellow-800"
                      : "bg-green-100 text-green-800"
                  }`}
                >
                  {moderationStats.flagged_messages?.pending || 0} Pending
                </span>
              </div>
              <p className="text-2xl font-semibold">
                {moderationStats.flagged_messages?.total || 0}
              </p>
              <p className="text-sm text-gray-500">
                {moderationStats.flagged_messages?.resolved || 0} resolved
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    (moderationStats.owner_reports?.pending || 0) > 0
                      ? "bg-yellow-100 text-yellow-800"
                      : "bg-green-100 text-green-800"
                  }`}
                >
                  {moderationStats.owner_reports?.pending || 0} Pending
                </span>
              </div>
              <p className="text-2xl font-semibold">
                {moderationStats.owner_reports?.total || 0}
              </p>
              <p className="text-sm text-gray-500">
                {moderationStats.owner_reports?.resolved || 0} resolved
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    (moderationStats.reviews?.flagged || 0) > 0
                      ? "bg-yellow-100 text-yellow-800"
                      : "bg-green-100 text-green-800"
                  }`}
                >
                  {moderationStats.reviews?.flagged || 0} Flagged
                </span>
              </div>
              <p className="text-2xl font-semibold">
                {moderationStats.reviews?.total || 0}
              </p>
              <p className="text-sm text-gray-500">
                {moderationStats.reviews?.removed || 0} removed
              </p>
            </div>
          </div>
        </Card>
